...
ScriptManager.shared.addResolver(async (scriptId, _caller) => {
  if (__DEV__) {
    const devServerUrl = Script.getDevServerURL(scriptId);
    console.log('Script.getDevServerURL', devServerUrl);
    return {
      url: devServerUrl,
      cache: false,
    };
  ...
  if (scriptId.includes('local')) {
    const fileSystemUrl = Script.getFileSystemURL(scriptId);
    console.log('Script.getFileSystemURL', fileSystemUrl);
    return {
      url: fileSystemUrl,
      cache: false,
    };
